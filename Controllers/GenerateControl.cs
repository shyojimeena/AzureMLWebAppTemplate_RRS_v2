    ...
    public static class GenerateControl
    {
        ...
        static private Control GenerateSliderIE(AMLParam param, string stepvalue)
        {
            ...
            lit.Text += string.Format("<div id=\"IEslider_{0}\" class = \"IEslider slider-displayIE\" data-begin=\"{1}\" data-end=\"{2}\" data-def=\"{3}\" data-step=\"{4}\"></div>",
                                                GenerateID(param), param.MinValue, param.MaxValue, param.DefaultValue, stepvalue);
        ...
        static private Control GenerateSliderHTML(AMLParam param, string stepvalue)
        {
            ...
            lit.Text += string.Format("<input id=\"slider_{0}\" type=\"range\" min=\"{1}\" max=\"{2}\" step=\"{3}\" class=\"slider slider-displayHTML form-control\"/>",
                                                GenerateID(param), param.MinValue, param.MaxValue, stepvalue);
        ...
        static private Control GenerateScriptSlideHTML(AMLParam param)
        {
            LiteralControl lit = new LiteralControl();
            lit.Text += "<script>";
            lit.Text += "document.getElementById(\"slider_{0}\").oninput = function(event){document.getElementById(\"{0}\").value = event.target.value};".Replace("{0}", GenerateID(param));
            lit.Text += "document.getElementById(\"{0}\").onchange = function(event){document.getElementById(\"slider_{0}\").value = event.target.value};".Replace("{0}", GenerateID(param));
            lit.Text += "</script>";
        ...
        static private Control GenerateScriptSlideIE(AMLParam param)
        {
            LiteralControl lit = new LiteralControl();
            lit.Text += "<script>";
            lit.Text += "document.getElementById(\"slider_{0}\").onchange = function(event){document.getElementById(\"{0}\").value = event.target.value};".Replace("{0}", GenerateID(param));
            lit.Text += "document.getElementById(\"{0}\").onchange = function(event){document.getElementById(\"slider_{0}\").value = event.target.value};".Replace("{0}", GenerateID(param));
            lit.Text += "</script>";
